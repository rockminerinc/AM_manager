...
class MY_pager{
	...
	var $num_tag_open= ' ';
	var $num_tag_close= '';
	var $lang_prev = 'Pre Page';
	var $lang_next = 'Nex Page';
	...
	function create_links()
	{
		...
		if($page_num > 1)
		{
			$output .= $this->first_tag_open.'<a '.$anchor_class.' href="'.base_url('/'.$page_url).'" >'.$this->lang_first.'</a>'.$this->first_tag_close;
		}
		//prev_link
		if($page_num > 1)
		{
		$n = $page_num - 1;
		$output .= $this->prev_tag_open.'<a '.$anchor_class.' href="'.base_url('/'.$page_url.'='.$n).'" >'.$this->lang_prev.'</a>'.$this->prev_tag_close;
		}
		//pages
		for($i=1;$i<=$pages;$i++)
		{
			...
			{
				$output .= $this->num_tag_open.'<a '.$anchor_class.' href="'.base_url('/'.$page_url.'='.$i).'" >'.$i.'</a>'.$this->num_tag_close;
			}
		}
		//next_link
		if($page_num < $pages)
		{
			$n = $page_num + 1;
			$output .= $this->next_tag_open.'<a '.$anchor_class.' href="'.base_url('/'.$page_url.'='.$n).'" >'.$this->lang_next.'</a>'.$this->next_tag_close;
		}
		//last_link
		if($page_num < $pages)
		{
			$output .= $this->last_tag_open.'<a '.$anchor_class.' href="'.base_url('/'.$page_url.'='.$pages).'" >'.$this->lang_last.'</a>'.$this->last_tag_close;
		}
